...
| `segmentation.py` | `split_segments()` — splits a sentence naming more than one symptom ("pet mein dard hai aur sar bhi dukh raha hai") into per-symptom pieces | Adding/removing a recognized separator word (`SEGMENT_SPLIT_PATTERN`), or changing the "don't over-split a short fragment" guard. `classifier.py`'s `predict()` runs the gibberish/coverage/classify pipeline once per segment (`_predict_segment()`) and merges the results. |
